...
class BrainNetworkSpike(nn.Module):
    def __init__(
        self,
        h=4096,
        in_dim=15724,
        ...
        self.T = T
        self.input_encoder = SeqToANNContainer(
            nn.Linear(in_dim, h * seq_len),
            nn.LayerNorm(h * seq_len),
            nn.Dropout(drop),
        )
        self.act = LIFSpike()
        self.mixer_blocks1 = nn.ModuleList(
            [self._spiking_mixer_block1(h, drop) for _ in range(n_blocks)]
        )
        self.mixer_blocks2 = nn.ModuleList(
            [self._spiking_mixer_block2(seq_len, drop) for _ in range(n_blocks)]
        )
        self.flatten = spiking_layer.Flatten(start_dim=2, end_dim=-1)
        # self.backbone_linear = layer.Linear(h * seq_len, out_dim, bias=True)
        self.backbone_linear = SeqToANNContainer(
            nn.Linear(h * seq_len, out_dim, bias=True)
        )
        self.clip_proj = SpikeProjector(
            in_dim=clip_size, out_dim=clip_size, h=clip_size
        )
    ...
    def _spiking_mixer_block1(self, h, drop):
        return nn.Sequential(
            SeqToANNContainer(nn.LayerNorm(h)),
            SpikeMLP(h, h, drop),
        )

    def _spiking_mixer_block2(self, seq_len, drop):
        return nn.Sequential(
            SeqToANNContainer(nn.LayerNorm(seq_len)),
            SpikeMLP(seq_len, seq_len, drop),
        )
    ...
    def forward(self, x):
        ...
        # Input encoding repeat over T times
        x_seq = x.unsqueeze(0).repeat(self.T, 1, 1, 1)  # [T, B, seq_len, h]
        # print(f"x shape:{x.shape}, x_seq shape: {x_seq.shape}")
        x_seq = self.input_encoder(x_seq)
        x_seq = self.act(x_seq)

        # Mixer blocks
        residual1 = x_seq
        residual2 = x_seq.permute(0, 1, 3, 2)
        # print(f"residual1 shape:{residual1.shape}, residual2 shape:{residual2.shape}")
        for block1, block2 in zip(self.mixer_blocks1, self.mixer_blocks2):
            x_seq = block1(x_seq) + residual1
            residual1 = x_seq
            x_seq = x_seq.permute(0, 1, 3, 2)

            x_seq = block2(x_seq) + residual2
            residual2 = x_seq
            x_seq = x_seq.permute(0, 1, 3, 2)

        backbone_seq = self.backbone_linear(x_seq)
        # print(f"backbone_seq{backbone_seq.shape}")
        backbone = backbone_seq.mean(dim=0)  # [B, out_dim]
        """
        if self.training:
            # 训练时：使用所有时间步的平均
            backbone_mean = backbone_seq.mean(dim=0)  # [B, features]
        else:
            # 推理时：使用最后一个时间步
            backbone_mean = backbone_seq[-1]  # [B, features]
        """
        backbone = backbone.reshape(batch_size, -1, self.clip_size)

        if self.clip_scale > 0:
            backbone_seq = backbone_seq.reshape(self.T, batch_size, -1, self.clip_size)
            # print(f"c backboneseq {backbone_seq.shape}")
            c = self.clip_proj(backbone_seq)
            c = c.mean(dim=0)
...
"""
class SpikeProjector(nn.Module):
    def __init__(self, in_dim, out_dim, h=2048):
        super(SpikeProjector, self).__init__()
        self.norm = SeqToANNContainer(nn.BatchNorm1d(in_dim))
        self.linearnorm1 = SeqToANNContainer(nn.Linear(in_dim, h), nn.BatchNorm1d(h))
        self.linearnorm2 = SeqToANNContainer(nn.Linear(h, h), nn.BatchNorm1d(h))
        self.linearout = SeqToANNContainer(nn.Linear(h, out_dim))
        self.act = LIFSpike()

    def forward(self, x):
        x = self.norm(x)
        x = self.act(x)
        x = self.linearnorm1(x)
        x = self.act(x)
        x = self.linearnorm2(x)
        x = self.act(x)
        x = self.linearout(x)
        return x
"""
...
class SpikeMLP(nn.Module):
    def __init__(self, in_dim, out_dim, drop):
        super().__init__()
        self.linear1 = SeqToANNContainer(nn.Linear(in_dim, out_dim))
        self.act = LIFSpike()
        self.drop = SeqToANNContainer(nn.Dropout(drop))
        self.linear2 = SeqToANNContainer(nn.Linear(out_dim, out_dim))
...
class Clipper(torch.nn.Module):
    def __init__(
        ...
        if clip_variant == "ViT-L/14" and hidden_state:
            ...
            image_encoder = CLIPVisionModelWithProjection.from_pretrained(
                sd_cache_dir, subfolder="image_encoder"
            ).eval()
    ...
    def embed_image(self, image):
        ...
        if self.norm_embs:
            if self.hidden_state:
                ...
                clip_emb = clip_emb / torch.norm(clip_emb[:, 0], dim=-1).reshape(
                    -1, 1, 1
                )
            else:
    ...
    def embed_text(self, text_samples):
        tokenizer = open_clip.get_tokenizer(self.clip_variant)
        clip_text = tokenizer(text_samples).to(self.device)
...
class BrainDiffusionPrior(DiffusionPrior):
    ...
    def forward(
        ...
        **kwargs,
        ...
        assert (
            exists(text) ^ exists(text_embed) ^ exists(voxel)
        ...
        if exists(voxel):
            assert exists(
                self.voxel2clip
            ), "voxel2clip must be trained if you wish to pass in voxels"
        ...
        loss, pred = self.p_losses(
            image_embed, times, text_cond=text_cond, *args, **kwargs
        )
...
class TrunkBlock(nn.Module):
    def __init__(self, feat_in, feat_out):
        ...
        self.conv1 = nn.Conv2d(
            feat_in, int(feat_out * 1.0), kernel_size=3, stride=1, padding=1, dilation=1
        )
        self.drop1 = nn.Dropout2d(p=0.5, inplace=False)
        self.bn1 = nn.BatchNorm2d(
            feat_in, eps=1e-05, momentum=0.25, affine=True, track_running_stats=True
        )

        torch.nn.init.xavier_normal_(
            self.conv1.weight, gain=torch.nn.init.calculate_gain("relu")
        )
...
class EncStage(nn.Module):
    def __init__(self, trunk_width=64, pass_through=64):
        ...
        self.drop1 = nn.Dropout2d(p=0.5, inplace=False)  ##
        self.bn1 = nn.BatchNorm2d(
            192, eps=1e-05, momentum=0.25, affine=True, track_running_stats=True
        )  ##
        ...
        ##
        torch.nn.init.xavier_normal_(
            self.conv3.weight, gain=torch.nn.init.calculate_gain("relu")
        )
        torch.nn.init.constant_(self.conv3.bias, 0.0)

    def forward(self, x):
        c3 = torch.nn.functional.relu(
            self.conv3(self.drop1(self.bn1(x))), inplace=False
        )
        c4a = self.conv4a(c3)
        c4b = self.conv4b(c4a)
        ...
        return [
            ...
            torch.cat(
                [c5a[:, : self.tw], c5b[:, : self.tw], c6a[:, : self.tw], c6b], dim=1
            ),
        ], c6b
...
class Torch_LayerwiseFWRF(nn.Module):
    ...
    def forward(self, fmaps):
        ...
        for fm, rf in zip(fmaps, self.rfs):  # , self.scales):
            ...
            # v     : [nv, space]
            phi += [
                torch.tensordot(g, f, dims=[[1], [2]]),
            ]  # apply pooling field and add to list.
            # phi : [nv, batch, features]
        Phi = torch.cat(phi, dim=2)
        if self.post_nl is not None:
            Phi = self.post_nl(Phi)
        vr = torch.squeeze(
            torch.bmm(Phi, torch.unsqueeze(self.w, 2))
        ).t() + torch.unsqueeze(self.b, 0)
        return vr
...
class GNet8_Encoder:
    ...
    def gnet8j_predictions(
        ...
        # allocate
        subject_image_pred = {
            s: np.zeros(shape=(len(image_data[s]), subject_nv[s]), dtype=np.float32)
            for s in subjects
        }
        ...
        # print(best_params)
        shared_model = GEncoder(
            np.array(checkpoint["input_mean"]).astype(np.float32),
        ...
        # example fmaps
        rec, fmaps, h = shared_model(
            torch.from_numpy(image_data[list(image_data.keys())[0]][:20]).to(device)
        )
        for s in subjects:
            sd = Torch_LayerwiseFWRF(
                fmaps,
                nv=subject_nv[s],
                pre_nl=_log_act_fn,
                post_nl=_log_act_fn,
            ...
            subject_image_pred[s] = self.subject_pred_pass(
                _pred_fn, shared_model, sd, image_data[s], batch_size
            )
        ...
        if isinstance(images, list):
            for i in range(len(images)):
                ...
                imagePil = (
                    images[i]
                    .convert("RGB")
                    .resize((w, h), resample=PIL.Image.Resampling.LANCZOS)
                )
                image = np.array(imagePil).astype(np.float32) / 255.0
                data.append(image)

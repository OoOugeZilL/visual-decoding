...
class GaussianDiffusionTrainer(nn.Module):
    ...
    def _model_forward(self, x_t, t, cond=None):
        if cond is None:
            return self.model(x_t, t)
        return self.model(x_t, t, cond)

    def forward(self, x_0, cond=None, reduction='none'):
        """
        Algorithm 1.
        """
        ...
        noise_pred = self._model_forward(x_t, t, cond=cond)
...
class GaussianDiffusionSampler(nn.Module):
    ...
    def p_mean_variance(self, x_t, t, cond=None):
        ...
        if self.sample_type=='ddpm':
            ...
            if self.mean_type == 'xprev':       # the model predicts x_{t-1}
                x_prev = self._model_forward(x_t, t, cond=cond)
                x_0 = self.predict_xstart_from_xprev(x_t, t, xprev=x_prev)
                model_mean = x_prev
            elif self.mean_type == 'xstart':    # the model predicts x_0
                x_0 = self._model_forward(x_t, t, cond=cond)
                model_mean, _ = self.q_mean_variance(x_0, x_t, t)
            elif self.mean_type == 'epsilon':   # the model predicts epsilon
                eps = self._model_forward(x_t, t, cond=cond)
    ...
    def forward(self, x_T, cond=None):
        x_t = x_T
        ...
        for n_count1,time_step in enumerate(self.t_list):
            ...
            if self.sample_type =='ddpm' or self.sample_type =='ddpm2':
                ...
                # no noise when t == 0
                if time_step > 0:
                    mean, log_var = self.p_mean_variance(x_t=x_t, t=t, cond=cond)
                ...
                x_t = self.p_mean_variance(x_t=x_t, t=t, cond=cond)

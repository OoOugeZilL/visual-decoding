...
class Spike_SelfAttention(nn.Module):
    ...
    def __init__(self, in_ch, num_heads=4):
        ...
        self.qkv_bn = nn.BatchNorm2d(in_ch * 3)

        # Output projection
        self.proj = nn.Conv2d(in_ch, in_ch, kernel_size=1, bias=False)
        self.proj_bn = nn.BatchNorm2d(in_ch)
    ...
    def forward(self, x):
        ...
        # Spike first (important for SNN stability)
        x = self.neuron(x)
        ...
        # QKV
        qkv = self.qkv_bn(self.qkv(x_flat))
        qkv = qkv.reshape(T, B, 3, C, H, W)
        ...
        v = v.reshape(T, B, self.num_heads, self.head_dim, N)

        # transpose for attention
        q = q.permute(0, 1, 2, 4, 3)  # [T,B,H,N,D]
        k = k.permute(0, 1, 2, 4, 3)  # [T,B,H,N,D]
        v = v.permute(0, 1, 2, 4, 3)  # [T,B,H,N,D]
        ...
        out = torch.matmul(attn, v)  # [T,B,H,N,D]
        ...
        out = self.proj_bn(self.proj(out_flat))
        out = out.reshape(T, B, C, H, W)
